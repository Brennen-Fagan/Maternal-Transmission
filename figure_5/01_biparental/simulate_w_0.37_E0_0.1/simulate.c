...
*														*
* Previously coded this on the Dell laptop in Ubuntu 18.04.  Am now running it on the HP laptop in Ubuntu 22.04.*
* The C compiler is now more picky about outputting pointers as integers.  Previously it just gave a warning.   *
* Now it gives a warning and an error.  So I've had to go through the code replacing %d with %p.                *
*														*
* 15.03.24.                                                                                                     *
*														*
*														*
* Makefile would no longer work until I had removed the call to the vogl library.                               *
*                                                                                                               *
* 25.11.21.													*
*														*
****************************************************************************************************************/
...
#define		method_vertical	3	/* method of vertical   transmission of symbionts
					   0: maternal
...
#define		method_horiz	1	/* method of horizontal transmission of symbionts
					   0: off
					   1: from environment
					   2: from another host (as in epidemics -- not implemented)	*/

#define		nspp		2	/* number of host species + 1					*/
#define		tmax		100.0/*200.0*/	/* maximum time for simulations					*/
#define		Nstart		656/*1000*/	/* initial population size of host species (must be > Nsymstart)*/
#define         Nsymstart       80/*10*/       /* initial number of sym+ hosts (use: 0 to check a sym- popn)	*/

/*Have tried to set the initial condition close to the equilibrium given by the ODE
0.1    E0
0.37   Wsym
0.1220 freq sym+ (80/656)
656    nhat
and starting the gene stopping male transmission a low frequency
*/

#define		fmodstart	0.02/*0.01*/     /* Starting frequency for gene stopping male transmission:
...
#define		niterate	1/*5000*/	/* number of iterations of stochastic process			*/
...
struct	rep
	{	int	sp;			/* host species						*/
		...
		double  e;			/* host prob p.u.t of encounter with symbiont in environ*/
...
struct	species_properties
	{	int	n;			/* number of hosts (total)		*/
		...
		double  e;			/* prob p.u.t of encounter with symbiont in environ*/

                double  E0;                     /* prob p.u.t of symb encounter in env	*/
		double	B0;			/* prob p.u.t.of birth (intrinsic)	*/
		double	D0;			/* prob p.u.t of death (intrinsic)	*/
		double	DD;			/* prob p.u.t of death (DD component)	*/
...
int	flag_vertical   = 0;			/* 1: detailed output for symbiont vertical transm  on 0: off	*/
int     flag_horizontal = 0;                    /* 1: detailed output for symbiont horizontl transm on 0: off  */
...
/*			printf("n:%d epidemic model for horizontal transmission still TO DO -- program halted\n");
*/			printf("method_horiz:%d epidemic model for horizontal transmission still TO DO -- program halted\n", method_horiz);
			exit(1);		
			break;
	}
	...
	fprintf(out, "  method_vertical: %4d    symb vertical: 0 mat; 1 pat;     2 bip\n",          method_vertical);
	fprintf(out, "  method_horiz:    %4d    symb horizont: 0 off; 1 environ; 2 from host\n",       method_horiz);
...
void	output_system_state()
{
	...
	{	nowP = param[sp].firstP;
		...
		fprintf(out, "   i       prevP        nowP       nextP  sex    birth  death   sym  wsym     sym_horiz   mod\n");
		i = 1;
		while (nowP != NULL)
/*		{	fprintf(out, "%4d %11d %11d %11d    %d   %6.3f %6.3f     %d %6.3f   %6.3f      %2d  \n", 
*/		{	fprintf(out, "%4d %p %p %p    %d   %6.3f %6.3f     %d %6.3f   %6.3f      %2d  \n", 
				i, nowP->prevP, nowP, nowP->nextP, nowP->sex, nowP->b, nowP->d, nowP->sym, nowP->wsym, nowP->e, nowP->mod);
			i = i + 1;
			nowP = nowP->nextP;
		}
	}
...
void	initialize_hosts()
{
	...
	for (i=0;   i<=param[sp].n;  ++i)
	{
		...
		nowP->wsym = 1.0;
            ...
            /***Initial values for encounter rate with symbiont in the environment (horizontal transmission)***/
		nowP->e = param[sp].E0;
...
void	individual_event_probabilities()
{
	...
	{	targP = param[sp].firstP;
		while (targP != NULL)
		{	
		    /***Probability of encounter p.u.t. (horizontal transfer) held fixed at param[sp].E0***/
...
repP	*pickP;
{
    ...
    /***Probability per unit time of symbiont encounter event set in initialize_parameters() (E0=0, when no horizontal transmission)***/
	for (sp=1; sp<nspp; ++sp)
	{	Esum[sp] = 0.0;
		nowP  = param[sp].firstP;
		while (nowP != NULL)
	...
	{	out = fopen("output.detail.dat", "a");
		...
		fprintf(out, "E/sum:%6.3f  B/sum:%6.3f  D/sum:%6.3f  rn1:%6.4f  event:%c    ", E/sum, B/sum, D/sum, rn1, *event);
/*              fprintf(out, "rn2:%6.4f  add:%6.4f  pickP:%d  firstP:%d\n", rn2, add, *pickP, param[nowP->sp].firstP);
...
*/		fprintf(out, "firstP:%p rn:%6.4f  add:%6.4f  dadP:%p\n", param[nowP->sp].firstP, rn, add, *dadP);
		fprintf(out, "  dadP:%p  sex:%d  sym:%d  mod:%d\n", *dadP, nowP->sex, nowP->sym, nowP->mod);
		fflush (out);
		fclose (out);
	}
}
...
*/		fprintf(out, "  pickP: %p  sym:%d  mod:%2d\n",  pickP,  pickP->sym,  pickP->mod);
		fprintf(out, "  dadP:  %p  sym:%d  mod:%2d\n",   dadP,   dadP->sym,   dadP->mod);
		fprintf(out, "  birthP:%p  sym:%d         \n", birthP, birthP->sym             );
		fflush (out);
		fclose (out);
	}
}
...
/***PROPERTIES OF NEWBORN INDIVIDUAL***/
    ...
    /***Neonate: horizontal transmission set by rules of the game***/
...
*/		fprintf(out, "  pickP: %p   pickP->sex:%d   pickP->sym:%d   pickP->mod:%2d   pickP->wsym:%6.4f\n",  pickP,  pickP->sex,  pickP->sym,  pickP->mod,  pickP->wsym);
		fprintf(out, "  dadP:  %p    dadP->sex:%d    dadP->sym:%d    dadP->mod:%2d    dadP->wsym:%6.4f\n",   dadP,   dadP->sex,   dadP->sym,   dadP->mod,   dadP->wsym);
		fprintf(out, "  birthP:%p  birthP->sex:%d  birthP->sym:%d  birthP->mod:%2d  birthP->wsym:%6.4f\n", birthP, birthP->sex, birthP->sym, birthP->mod, birthP->wsym);
		fflush (out);
		fclose (out);
	}
}
...
*/		fprintf(out, "  pickP: %p  pickP->sex:%d  pickP->sym:%d  pickP->mod:%d  pickP->wsym:%6.4f\n",  pickP,  pickP->sex,  pickP->sym,  pickP->mod,  pickP->wsym);
		fflush (out);
		fclose (out);
	}
}
...
*														*
* This is simple: it just changes the host from sym- to sym+, if the host was previoiusly sym-			*
...
repP	pickP;		/* pointer to host encountering a symbiont */
{
    ...
    /***Assign the symbiont status and fitness to the chosen individual*/
        if (pickP->sym == 0)
        {       pickP->sym  = 1;
                pickP->wsym = Wsym;
	}
    ...
    /***Details of calculation***/
        ...
        {       out = fopen("output.detail.dat", "a");
                fprintf(out, "\nTime:%9.6f  State at end of symbiont horiz_trans procedure\n", t);
/*              fprintf(out, "  pickP:%d  sex:%d  sym:%d  mod:%d   pickP->wsym:%6.4f\n",  pickP,  pickP->sex,  pickP->sym,  pickP->mod,  pickP->wsym);
*/              fprintf(out, "  pickP:%p  sex:%d  sym:%d  mod:%d   pickP->wsym:%6.4f\n",  pickP,  pickP->sex,  pickP->sym,  pickP->mod,  pickP->wsym);
                fflush (out);
                fclose (out);
	}
}
...
Wsym = 0.37;
		...
		{	count_fixed += 1;
			out = fopen("output.invasions.fixed.dat", "a");
			fprintf(out, "%6d   %6d	  %8.6f	  %4d	%4d   %6.4f   %6.2f  %6d", iter, count_fixed, Wsym, param[sp].n, param[sp].nsym, param[sp].fsym, t, nevent);
			fprintf(out, "\n");
			fflush (out);
			fclose (out);
		}
		...
		{	count_lost += 1;
			out = fopen("output.invasions.lost.dat", "a");
			fprintf(out, "%6d   %6d	  %8.6f	  %4d	%4d   %6.4f   %6.2f  %6d", iter, count_lost, Wsym, param[sp].n, param[sp].nsym, param[sp].fsym, t, nevent);
			fprintf(out, "\n");
			fflush (out);
			fclose (out);
		}
	    /***Add to list of iterations in which symbionts: undecided***/
		if (t >= tmax)
		{	count_undecided += 1;
			out = fopen("output.invasions.undecided.dat", "a");
			fprintf(out, "%6d   %6d	  %8.6f	  %4d	%4d   %6.4f   %6.2f  %6d", iter, count_undecided, Wsym, param[sp].n, param[sp].nsym, param[sp].fsym, t, nevent);
			fprintf(out, "\n");
			fflush (out);
			fclose (out);
		}
