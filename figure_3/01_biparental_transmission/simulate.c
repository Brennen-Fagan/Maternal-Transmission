/****************************************************************************************************************
* UPDATED C CODE FOR SYMBIONT-SIEVE PAPER  (11.04.23)                                                           *
*	                                                                                                        *
* Host stochastic birth-death process with vertical transmission of symbionts.                                  *
* Includes procedures allowing additional horizontal tranmission of symbionts.                                  *
* Includes procedures allowing evolution of a gene modifying vertical transmision through males.                *
*                                                                                                               *
* For this new work on the host microbiome, I've worked from the published C code (28.07.22)                    *
* There are now two microbiome vectors for each host of length Nmic; this allows Nmic-1 symbiont taxa           *
* (the taxa are indexed 1, 2, ..., Nmic-1):                                                                     *
*     mic[] takes values:  0:symbiont absent 1:symbiont present                                                 *
*   w1mic[] gives a contribution to host fitness w1sym, with no       interactions between symbiont taxa        *
*   w2mic[] gives a contribution to host fitness w2sym, with pairwise interactions between symbiont taxa        *
*                                                                                                               *
* New procedure: initialise_hosts_microbiome()  constructs host microbiome for all hosts at start of simulation	*
* New procedure: microbiome_transmssion()       constructs host microbiome at birth                             *
* New procedure: host_fitness_microbiome()      sets each host fitness from its microbiome                      *
* New procedure: output_timeseries_microbiome() for output of microbiome timeseries				*
*	                                                                                                        *
* Have removed two old procedures which dealt with single symbiont taxa; can now just set Nmic = 2 for this:    *
* procedure: initialise_hosts():       replaced with initialise_hosts_microbiome()                              *
* procedure: transmission_symbiont():  replaced with microbiome_transmission()                                  *
* Have checked the new code gives results consistent with figures 3, 4, 5 in the orginal submission to Nat Comms*
*                                                                                                               *
* Still have two separate procedures for time-series output:                                                    *
* procedure: output_timeseries_single()         Nmic = 2                                                        *
* procedure: output_timeseries_microbiome()     Nmic > 2                                                        *
*                                                                                                               *
* Found an error in earlier code for birth(): the father must be chosen before the birth happens, otherwise     *
* the list contains the newborn individual when the father is picked , and the neonate could be its father!     *
*                                                                                                               *
*                                                                                                               *
*                                                                                                               *
* MODIFICATIONS IN 2024   (0.03.24)                                                                             *
*                                                                                                               *
* New procedure: microbiome_size()        constructs histograms of number of microbial taxa in hosts            *
*                                         at times chosen by user                                               *
*                                                                                                               *
* New information included on each host:  Smic: number of symbiont taxa in host microbiome                      *
* Seed for random numbers as in 2023:     7597                                                                  *
*                                                                                                               *
* Choosing a time for histogramming:      goto simulate() and choose time to construct histogram                *
*                                         this terminates the run (line 1520)                                   *
*                                                                                                               *
****************************************************************************************************************/
...
#define     method_fitness   1      /* effect of microbiome taxa on host fitness
                                       ...
                                       2: interaction between pairs of taxa
                                       3: ring of pairs across all taxa                              */
...
#define     method_vertical  2      /* method of vertical transmission of symbionts
                                       0: maternal
...
struct  rep
        {   int     sp;                  /* host species                                             */
            ...
            int     Smic;                /* number of symbiont taxa in host microbiome               */
...
double  freq_Smic[2][Nmic];         /* array with freq distrib of # microbe taxa per host  */
...
repP	pickP;
{
	...
	switch(method_fitness)
	{
                ...
                case 2: /*pair interaction between each symbiont and the next one in list*/
                        pickP->whost = 0.0;
                        countsym     = 0;
                        for (sm=1; sm<Nmic-1; ++sm)             /*loop for interior symbiont taxa*/
                        if  (pickP->mic[sm] == 1)
                        {       toadd = pickP->w1mic[sm];
                                if  (pickP->mic[sm+1] == 1)     toadd *= pickP->w2mic[sm] * pickP->w1mic[sm+1];
                                pickP->whost += toadd;
                                countsym     += 1;
                        }
                        sm = Nmic-1;                            /*symbiont taxon (Nmic-1)*/
                        if  (pickP->mic[Nmic-1] == 1)
                        {       toadd = pickP->w1mic[Nmic-1];
                                if  (pickP->mic[1] == 1)        toadd = pickP->w2mic[sm] * pickP->w1mic[1];	
                                pickP->whost += toadd;
                                countsym     += 1;
                        }
	                if (countsym == 0)   pickP->whost = 1;
                ...
                case 3: /*Ring of pairs across all symbiont taxa (each affected by previous and next taxon in list*/
                        pickP->whost = 0.0;
                        countsym     = 0;
                        for (sm=2; sm<Nmic-1; ++sm)             /*loop for interior symbiont taxa*/

                        if  (pickP->mic[sm] == 1)
                        {       toadd = pickP->w1mic[sm];
                                if  (pickP->mic[sm-1] == 1)     toadd *= pickP->w2mic[sm-1];
                                if  (pickP->mic[sm+1] == 1)     toadd *= pickP->w2mic[sm+1];
                                pickP->whost += toadd;
                                countsym     += 1;
                        }
                        sm = 1;                                 /*symbiont taxon 1*/
                        if  (pickP->mic[sm] == 1)
                        {       toadd = pickP->w1mic[sm];
                                if  (pickP->mic[Nmic-1] == 1)   toadd *= pickP->w2mic[Nmic-1];
                                if  (pickP->mic[sm+1]   == 1)   toadd *= pickP->w2mic[sm+1];
                                pickP->whost += toadd;
                                countsym     += 1;
                        }
                        ...
                        {       toadd = pickP->w1mic[sm];
                                if  (pickP->mic[sm-1] == 1)     toadd *= pickP->w2mic[sm-1];
                                if  (pickP->mic[1]    == 1)     toadd *= pickP->w2mic[1];
                                pickP->whost += toadd;
                                countsym     += 1;
                        }
                        if (countsym == 0)   pickP->whost = 1;    
...
*/      printf("\n");
    ...
    /***Record results to file***/
        ...
        if (t > tmax)
        {       fprintf(out, "\n");
                fprintf(out, "#time                     w1[i]:");
                for(sm=1; sm<Nmic; ++sm)    fprintf(out, "%5.2f ", param[sp].w1mic[sm]);    fprintf(out, "\n");
                fprintf(out, "#                         w2[i]:");
                for(sm=1; sm<Nmic; ++sm)    fprintf(out, "%5.2f ", param[sp].w2mic[sm]);    fprintf(out, "\n");

                fprintf(out, "#           w1[i]*w2[i]*w1[i+1]:");
                for(sm=1; sm<Nmic-1; ++sm)  
                             fprintf(out, "%5.2f ",  param[sp].w1mic[sm] * param[sp].w2mic[sm] * param[sp].w1mic[sm+1]);
                sm = Nmic-1; fprintf(out, "%5.2f ",  param[sp].w1mic[sm] * param[sp].w2mic[sm] * param[sp].w1mic[1]);
                fprintf(out, "\n");

                fprintf(out, "#         w1[i]*w2[i-1]*w2[i+1]:");
                sm = 1;      fprintf(out, "%5.2f ",  param[sp].w1mic[sm] * param[sp].w2mic[Nmic-1] * param[sp].w2mic[sm+1]);
                for(sm=2; sm<Nmic-1; ++sm)  
                             fprintf(out, "%5.2f ",  param[sp].w1mic[sm] * param[sp].w2mic[sm-1]   * param[sp].w2mic[sm+1]);
                sm = Nmic-1; fprintf(out, "%5.2f ",  param[sp].w1mic[sm] * param[sp].w2mic[sm-1]   * param[sp].w2mic[1]);
                fprintf(out, "\n\n");

                switch(method_fitness)
                {       case 1: fprintf(out, "Using method_fitness: 1: w1[i]                  additive\n");            break;
                        case 2: fprintf(out, "Using method_fitness: 2: w1[i]*w2[i]*w1[i+1]    pair interactions\n");   break;
                        case 3: fprintf(out, "Using method_fitness: 3: w1[i]*w2[i-1]*w2[i+1]  pair interactions\n");   break;
	        }
                fprintf(out, "\n");

                fprintf(out, "Warning: mean values assume all symbiont taxa are present in all hosts: divides summation by Nmic-1\n");
                sum = 0.0;
                for(sm=1; sm<Nmic; ++sm)    sum += param[sp].w1mic[sm];    
                fprintf(out, "# mean singles:  %6.3f  :      sum_i (w1[i])                     / (Nmic-1)", sum/(Nmic-1));
                fprintf(out, "\n");

                sum = 0.0;
                for(sm=2; sm<Nmic-1; ++sm)  sum += param[sp].w1mic[sm] * param[sp].w2mic[sm] * param[sp].w1mic[sm+1];    
                sm = Nmic-1;                sum += param[sp].w1mic[sm] * param[sp].w2mic[sm] * param[sp].w1mic[1];
                fprintf(out, "# mean in pairs: %6.3f  :      sum_i (w1[i] * w2[i]   * w1[i+1]) / (Nmic-1)", sum/(Nmic-1));
                fprintf(out, "\n");

                sum = 0.0;
                sm = 1;                     sum += param[sp].w1mic[sm] * param[sp].w2mic[Nmic-1] * param[sp].w2mic[sm+1];
                for(sm=2; sm<Nmic-1; ++sm)  sum += param[sp].w1mic[sm] * param[sp].w2mic[sm-1]   * param[sp].w2mic[sm+1];    
                sm = Nmic-1;                sum += param[sp].w1mic[sm] * param[sp].w2mic[sm-1]   * param[sp].w2mic[1];
                fprintf(out, "# mean in pair rings: %6.3f  : sum_i (w1[i] * w2[i-1] * w2[i+1]) / (Nmic-1)", sum/(Nmic-1));
                fprintf(out, "\n\n\n");
	}
        fflush (out);
        fclose (out);
...
/****************************************************************************************************************
* Output frequency distribution of number of hosts with 0, 1, 2, ..., (Nmic-1) symbiont taxa at a given time    *
****************************************************************************************************************/

void    microbiome_size()
{
        FILE    *out;
        int     sp, sm, i, j;
        repP    nowP;

        out = fopen("output.freq.microbiome.size.dat", "a");
	
        for (sp=1; sp<nspp; ++sp)
        {
            /***Countup number of microbial taxa in each host***/
                nowP = param[sp].firstP;
                while (nowP != NULL)
                {       nowP->Smic = 0;
                        for (sm=1; sm<Nmic; ++sm)   nowP->Smic = nowP->Smic + nowP->mic[sm];
                        nowP = nowP->nextP;
		}

            /***Construct frequency distribution for number of microbial taxa per host***/
                for (sm=0; sm<Nmic; ++sm)       freq_Smic[0][sm] = sm;
                for (sm=0; sm<Nmic; ++sm)       freq_Smic[1][sm] = 0;
                nowP = param[sp].firstP;
                while (nowP != NULL)
                {       freq_Smic[1][nowP->Smic] += 1;
                        nowP = nowP->nextP;
		}

            /***Record frequency distribution number of microbial taxa per host***/
                fprintf(out, "# taxa  Frequency(n)   propn\n");
                for (sm=0; sm<Nmic; ++sm)
                {       fprintf(out, "%4.0f        ", freq_Smic[0][sm]);
                        fprintf(out, "%4.0f       ",  freq_Smic[1][sm]);
                        fprintf(out, "%6.4f",         freq_Smic[1][sm] / param[sp].n);
                        fprintf(out, "\n");
		}
	}

    /***Record host by host calculation***/
        for (sp=1; sp<nspp; ++sp)
        {       fprintf(out, "\n# Microbiome strings for all %d hosts \n", param[sp].n);
                i = 0;
                nowP = param[sp].firstP;
                while (nowP != NULL)
                {       i = i + 1;
                        ...
                        for (sm=1; sm<Nmic; ++sm)   nowP->Smic = nowP->Smic + nowP->mic[sm];
                        fprintf(out, "# %4d   ", i);
                        for (sm=1; sm<Nmic; ++sm)   fprintf(out, "%d", nowP->mic[sm]);
                        fprintf(out, "%4d   ", nowP->Smic);
                        fprintf(out, "\n");
                        nowP = nowP->nextP;
		}
        }
...
*/      {
           ...
           /***Frequency distribution of microbiome size at a chosen time (computation terminated)***/
               if (t>4.0-0.001)
               {        microbiome_size();
                        exit(1);
	       }
...
int     main()
{
    ...
    /***Initialize output files (ensures they are empty at the start)***/
        ...
        fclose(fopen("output.freq.microbiome.size.dat","w"));
