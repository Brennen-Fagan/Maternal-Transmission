/****************************************************************************************************************
* C code from symbiont-sieve paper                                                                              *
*                                                                                                               *
...
#define     tmax            200.0   /* maximum time for simulations                                  */
#define     Nstart          1000    /* initial population size of host species (must be > Nsymstart) */
#define     Nsymstart       10      /* initial number of sym+ hosts (use: 0 to check a sym- popn)    */

#define     fmodstart       0.00    /* Starting frequency for gene stopping male transmission applies
...
#define     niterate        5000    /* number of iterations of stochastic process                    */
...
int     flag_timeseries = 0;        /* 1: output for timeseries of realization on  0: off  */
    ...
    /***Output bidirectional list of host individuals by species***/
        ...
        {       nowP = param[sp].firstP;
                ...
                {       fprintf(out, "%4d %p %p %p    %d   %6.3f %6.3f     %d %6.3f   %6.3f      %2d\n",
                                ...
                                nowP->sex, nowP->b, nowP->d, nowP->sym, nowP->wsym, nowP->e, nowP->mod);
                        i = i + 1;
                        nowP = nowP->nextP;
                }
        }
...
void    initialize_hosts()
{
    ...
    /***Give some hosts a symbiont and modify host fitness accordingly (no. of hosts with symbiont is Nsymstart)***/
    /***These hosts are at the top of the list***/
        ...
        {       i = 1;
                ...
                {       nowP->sym  = 1;
                        ...
                        switch(method_sym)
                        {
                                case 1: /*intrinsic host birth rate  becomes B0*wsym (applies only to females)*/
                                        switch(nowP->sex)
                                        {       case 0: /*female*/
                                ...
                                case 2: /*intrinsic host death rate becomes D0*wsym (applies to females and males*/
                                        nowP->d = param[sp].D0 * nowP->wsym + param[sp].DD * param[sp].n;
                                        break;

                                case 3: /*intrinsic host death rate becomes D0/wsym (applies to females and males*/
                                        nowP->d = param[sp].D0 / nowP->wsym + param[sp].DD * param[sp].n;
                                        break;

                                case 4: /*total host death rate becomes D/wsym (applies to females and males*/
    ...
    /***Loop for setting probabilties p.u.t. for individuals***/
        ...
        {       targP = param[sp].firstP;
                while (targP != NULL)
                {
                    ...
                    /***Prob. of birth p.u.t. fixed at param[sp].B0 * wsym;  targP->b entered when targP initialized***/
                    ...
                    /***Prob. of death p.u.t. depends on n and on whether host is sym+ ***/
                        switch(method_sym)
                        {
                                ...
                                case 3: /*intrinsic host death rate beomes D0/wsym (applies to females and males)*/
                                        targP->d = param[sp].D0 / targP->wsym + param[sp].DD * param[sp].n;
                                        break;
...
/***PROPERTIES OF NEWBORN INDIVIDUAL***/
    ...
    /***Neonate: random father from choose_dad(): needed for symbiont trans. and for evolution of modifier gene***/
    ...
    /***Neonate: horizontal transmission set by rules of the game (E0)***/
...
void    simulate()
{
    ...
    /***Use one of these loops; comment out the other one***/
    /***Loop to run for a fixed time tmax -- needed for dislaying a single time series***/
/*        while (t<=tmax)
*/
